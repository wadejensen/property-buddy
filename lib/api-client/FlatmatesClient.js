...
class FlatmatesClient {
    ...
    async getListings(listingSearchOpts) {
        const data = await this._mapMarkers(listingSearchOpts)
        const flatmatesListings = data.matches

        return flatmatesListings.map(normaliseListing)
